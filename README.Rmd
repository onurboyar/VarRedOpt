...
The increase in computing power has been making us capable to run bigger simulations. We can choose
...
order to get more robust results with the computing power at hand.

The reliability of the simulation lies is the variance of the simulation. As the simulation size increase,
...
variance reduction (VR) algorithm comes to our help.

A Variance Reduction Algorithm is an algorithm that behaves like the simulation itself. These algorithms
uses simulations as an input and returns another simulation with almost the same expected value and less
variance value.

In this library, we are sharing different VR algorithms as a framework. Antithetic Variates, Inner Control
Variates, Outer Control Variates and Importance Sampling algorithms are applied and presentended as
...
Another aspect need to be mentioned of the simulate.outer() function is the q.outer parameter. In the
...
This function, _myq_, takes two parameters. The length of the point vector and the dimension of the z.matrix should be the same in order to find euclidean distance between these points. Note that returning value type have to be list.
